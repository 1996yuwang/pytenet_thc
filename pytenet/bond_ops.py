...
__all__ = ['retained_bond_indices', 'split_matrix_svd', 'qr', 'retained_bond_indices_max_bond', 'split_matrix_svd_max_bond']
...
def retained_bond_indices_max_bond(s, tol, max_bond):
    """
    Indices of retained singular values based on given tolerance and maximum bond dimensions.
    It at most return max_bond (an integer) signular values.
    """
    w = np.linalg.norm(s)
    if w == 0:
        return np.array([], dtype=int)
    ...
    sort_idx = np.argsort(s)
    s[sort_idx] = np.cumsum(s[sort_idx])
    retain_list = np.where(s > tol)[0]
    
    if len(retain_list) > max_bond:
        return (sort_idx[-max_bond:])

    else: 
        return retain_list
    
    
def split_matrix_svd_max_bond(A, q0, q1, tol, max_bond):
    """
    Split a matrix by singular value decomposition,
    taking block sparsity structure dictated by quantum numbers into account,
    and truncate small singular values based on tolerance.
    ...
    # truncate small singular values
    idx = retained_bond_indices_max_bond(s, tol, max_bond)
    u = u[:, idx]
    v = v[idx, :]
    s = s[idx]
    q = q[idx]
